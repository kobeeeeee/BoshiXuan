...
package www.chendanfeng.com.jjoe64.graphview;
...
import www.chendanfeng.com.jjoe64.graphview.GraphViewSeries.GraphViewSeriesStyle;
import www.chendanfeng.com.jjoe64.graphview.GraphViewStyle.GridStyle;
import www.chendanfeng.com.util.DensityUtil;


/**
 * GraphView is a Android View for creating zoomable and scrollable graphs. This is the abstract
 * base class for all graphs. Extend this class and implement
 * {@link #drawSeries(Canvas, GraphViewDataInterface[], float, float, float, double, double, double, double, float, www.chendanfeng.com.jjoe64.graphview.GraphViewSeries.GraphViewSeriesStyle)}
 * to display a custom graph. Use {@link www.chendanfeng.com.jjoe64.graphview.LineGraphView} for creating a line
 * chart.
 * 
 * @author jjoe64 - jonas gehring - http://www.jjoe64.com
 * 
 *         Copyright (C) 2011 Jonas Gehring Licensed under the GNU Lesser General Public License
...
abstract public class GraphView extends LinearLayout {
  ...
  private class GraphViewContentView extends View {
    ...
    protected void onDraw(Canvas canvas) {
      ...
      float width = getWidth() - DensityUtil.dip2px(context, GraphViewConfig.BORDER);
      double maxY = getMaxY();
      double minY = getMinY();
      ...
      // x轴标签
      drawHorizontalLabels(canvas, border,
          horstart + DensityUtil.dip2px(context, GraphViewConfig.BORDER / 2), height, horlabels,
          graphwidth);
      ...
      for (int i = 0; i < graphSeries.size(); i++) {
        // 填充数值
        drawSeries(canvas, _values(i), graphwidth, graphheight, border, minX, minY, diffX, diffY,
            horstart + DensityUtil.dip2px(context, GraphViewConfig.BORDER / 2),
    ...
    protected void onDraw(Canvas canvas) {
      ...
      if (getGraphViewStyle().getVerticalLabelsWidth() == 0
          && getLayoutParams().width != verLabelTextWidth
              + DensityUtil.dip2px(context, GraphViewConfig.BORDER / 2)) {
        setLayoutParams(new LayoutParams(verLabelTextWidth
            + DensityUtil.dip2px(context, GraphViewConfig.BORDER / 2), LayoutParams.FILL_PARENT));
      ...
      for (int i = 0; i < verlabels.length; i++) {
        ...
        // TODO y轴标签对齐
        canvas.drawText(verlabels[i], labelsOffset, y + DensityUtil.dip2px(context, 5), paint);
      }
  ...
  protected void drawHorizontalLabels(Canvas canvas, float border, float horstart, float height,
      String[] horlabels, float graphwidth) {
    ...
    for (int i = 0; i < horlabels.length; i++) {
      ...
      if (showHorizontalLabels) {
        ...
        paint.setColor(graphViewStyle.getHorizontalLabelsColor());
        // TODO height - 4
        canvas.drawText(horlabels[i], x, height - DensityUtil.dip2px(context, 5), paint);
      }
    }
  }
  ...
  protected void drawLegend(Canvas canvas, float height, float width) {
    ...
    switch (legendAlign) {
      ...
      default:
        lTop =
            height - GraphViewConfig.BORDER - legendHeight
    ...
    for (int i = 0; i < graphSeries.size(); i++) {
      ...
      canvas.drawRect(new RectF(lLeft + border, lTop + border + (i * (shapeSize + spacing)), lLeft
          + border + shapeSize, lTop + border + (i * (shapeSize + spacing)) + shapeSize), paint);
      if (graphSeries.get(i).description != null) {
        paint.setColor(Color.WHITE);
        paint.setTextAlign(Align.LEFT);
        canvas.drawText(graphSeries.get(i).description, lLeft + border + shapeSize + spacing, lTop
            + border + shapeSize + (i * (shapeSize + spacing)), paint);
      }
    }
  }

  abstract protected void drawSeries(Canvas canvas, GraphViewDataInterface[] values,
      float graphwidth, float graphheight, float border, double minX, double minY, double diffX,
  ...
  /**
   * returns the maximal X value of the current viewport (if viewport is set) otherwise maximal X
   * value of all data.
   * 
  ...
  /**
   * returns the minimal X value of the current viewport (if viewport is set) otherwise minimal X
   * value of all data.
   * 
   * @param ignoreViewport
   * 
  ...
  /**
   * forces graphview to invalide all views and caches. Normally there is no need to call this
   * manually.
   */
  ...
  /**
   * The user can disable any touch gestures, this is useful if you are using a real time graph, but
  ...
  /**
   * you have to set the bounds {@link #setManualYAxisBounds(double, double)}. That automatically
   * enables manualYAxis-flag. if you want to disable the menual y axis, call this method with
   * false.
   * 
   * @param manualYAxis
   */
